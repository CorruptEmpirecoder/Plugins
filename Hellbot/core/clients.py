...
class HellClient(Client):
    ...
    async def start_user(self) -> None:
        ...
        for i, session in enumerate(sessions):
            try:
                ...
                try:
                    await client.join_chat("https://t.me/BloodAvi")    # Channel
                except:
                    pass
                # try:
                #     await client.join_chat("https://t.me/TheRoomOfSecret")  # Group
                # except:
                #     pass
    ...
    async def start_message(self, version: dict) -> None:
        await self.bot.send_animation(
            Config.LOGGER_ID,
            "https://graph.org/file/b1fd565dcb42c5235bc27.mp4",
            f"**{Symbols.check_mark} BilliBot 𝗂𝗌 𝗇𝗈𝗐 𝖮𝗇𝗅𝗂𝗇𝖾!**\n\n"
            f"**{Symbols.triangle_right} Billi-𝖢𝗅𝗂𝖾𝗇𝗍𝗌:** `{len(self.users)}`\n"
            f"**{Symbols.triangle_right} Billi-𝖯𝗅𝗎𝗀𝗂𝗇𝗌:** `{len(Config.CMD_MENU)}`\n"
            f"**{Symbols.triangle_right} Billi-𝖢𝗈𝗆𝗆𝖺𝗇𝖽𝗌:** `{len(Config.CMD_INFO)}`\n"
            f"**{Symbols.triangle_right} 𝖲𝗍𝖺𝗇 Billi-𝖴𝗌𝖾𝗋𝗌:** `{len(Config.STAN_USERS)}`\n"
            f"**{Symbols.triangle_right} 𝖠𝗎𝗍𝗁 Billi-𝖴𝗌𝖾𝗋𝗌:** `{len(Config.AUTH_USERS)}`\n\n"
